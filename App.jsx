...
import {
  ArrowRight, BadgeCheck, Banknote, Bird, Check, ChevronRight, Clock3,
  ...
  async function submit(e){
    ...
    try{
      if(mode==="login"){const {error}=await supabase.auth.signInWithPassword({email,password});if(error)throw error;}
        ...
        const {data,error}=await supabase.auth.signUp({email,password,options:{data:{full_name:name,phone,referred_by:ref.trim().toUpperCase()||null}}});
...
function BuyModal({duck,onClose,onDone}){
 ...
 return <Modal title={`Buy ${duck.name}`} onClose={onClose}><div className="buy-summary"><Bird size={45}/><div><b>{duck.name}</b><span>{money(duck.price)} · {duck.egg_interval_hours}h cycle · {duck.lifetime_days} days</span></div></div>{msg&&<Notice type={msg.startsWith("Payment")?"success":"error"}>{msg}</Notice>}<form onSubmit={submit}><Select label="Payment method" value={method} onChange={e=>setMethod(e.target.value)} required>{methods.map(m=><option key={m.id}>{m.name}</option>)}</Select>{methods.find(m=>m.name===method)&&<div className="payment-detail">{JSON.stringify(methods.find(m=>m.name===method),["account_name","account_number","instructions"],2)}</div>}<Field label="Transaction ID (TID)" value={tid} onChange={e=>setTid(e.target.value)} required/><label className="field"><span>Payment slip</span><input type="file" accept="image/*,.pdf" onChange={e=>setSlip(e.target.files?.[0]||null)} required/></label><Button disabled={busy}>{busy?<RefreshCw className="spin"/>:<Check/>}Submit payment</Button></form></Modal>
}
...
function ReferralPage({profile,refresh}){
 ...
 useEffect(()=>{(async()=>{try{await supa(supabase.rpc("claim_due_referral_eggs"));setRows(await supa(supabase.from("referral_rewards").select("*,referred:referred_user_id(full_name,customer_id)").eq("referrer_id",profile.id).order("created_at",{ascending:false})))}catch{}})()},[refresh,profile.id]);
...
function WithdrawPage({refresh}){
 const [w,setW]=useState(null),[opts,setOpts]=useState([]),[form,setForm]=useState({option:"",method:"JazzCash",holder:"",account:""}),[msg,setMsg]=useState("");
 useEffect(()=>{(async()=>{try{const {data:{user}}=await supabase.auth.getUser();const [w,o]=await Promise.all([supa(supabase.from("egg_wallets").select("*").eq("user_id",user.id).maybeSingle()),supa(supabase.from("withdrawal_options").select("*").eq("active",true).order("min_eggs"))]);setW(w);setOpts(o||[]);if(o?.[0])setForm(f=>({...f,option:o[0].id}))}catch(e){setMsg(errText(e))}})()},[refresh]);
 const opt=opts.find(x=>x.id===form.option);
 async function submit(e){e.preventDefault();setMsg("");try{await supa(supabase.rpc("create_withdrawal",{withdrawal_eggs:opt.min_eggs,withdrawal_amount:opt.min_eggs*Number(opt.rate_per_egg),withdrawal_method:form.method,withdrawal_account:form.account,withdrawal_holder_name:form.holder,withdrawal_rate:Number(opt.rate_per_egg)}));setMsg("Withdrawal request submitted for admin review.");}catch(e){setMsg(errText(e))}}
 return <><PageTitle eyebrow="WITHDRAW" title="Withdraw eggs" desc="Select a rate, enter your payout details, and submit for approval."/><Card className="balance-card"><span>Available eggs</span><b>{w?.total_eggs||0}</b><small>{money(w?.balance)}</small></Card><Card><div className="section-head"><h2>Withdrawal rate</h2></div>{msg&&<Notice type={msg.startsWith("Withdrawal")?"success":"error"}>{msg}</Notice>}<form onSubmit={submit}><Select label="Egg range" value={form.option} onChange={e=>setForm({...form,option:e.target.value})}>{opts.map(o=><option key={o.id} value={o.id}>{o.min_eggs}–{o.max_eggs} eggs · {money(o.rate_per_egg)}/egg</option>)}</Select><Select label="Payout method" value={form.method} onChange={e=>setForm({...form,method:e.target.value})}><option>JazzCash</option><option>Easypaisa</option><option>Bank</option></Select><Field label="Account holder name" value={form.holder} onChange={e=>setForm({...form,holder:e.target.value})} required/><Field label="Account number" value={form.account} onChange={e=>setForm({...form,account:e.target.value})} required/>{opt&&<div className="withdraw-total"><span>Request</span><b>{opt.min_eggs} eggs → {money(opt.min_eggs*Number(opt.rate_per_egg))}</b></div>}<Button disabled={!opt}><Banknote/>Submit withdrawal</Button></form></Card></>
}

function ProfilePage({profile}){
 const [name,setName]=useState(profile?.full_name||""),[phone,setPhone]=useState(profile?.phone||""),[msg,setMsg]=useState("");
 async function save(e){e.preventDefault();try{await supa(supabase.from("profiles").update({full_name:name,phone}).eq("id",profile.id));setMsg("Profile updated.")}catch(e){setMsg(errText(e))}}
...
function AdminDeposits({refresh}){
 ...
 const load=()=>supa(supabase.from("payment_requests").select("*,profiles(full_name,phone,customer_id),duck_purchases(*,ducks(name))").order("created_at",{ascending:false})).then(setRows).catch(e=>setMsg(errText(e)));
 useEffect(load,[refresh]);
 ...
 return <><PageTitle eyebrow="REVIEW" title="Deposits" desc="Approve or reject payment slips and TIDs."/><Card>{msg&&<Notice type="error">{msg}</Notice>}<AdminTable rows={rows} columns={["Customer","Duck","Amount","TID","Status","Action"]} render={r=><><div><b>{r.profiles?.full_name||"Customer"}</b><span>{r.profiles?.customer_id||r.profiles?.phone||""}</span></div><span>{r.duck_purchases?.ducks?.name||"Duck"}</span><span>{money(r.amount)}</span><span>{r.transaction_id}</span><span className={`status ${r.status==="approved"?"ok":r.status==="rejected"?"bad":"wait"}`}>{r.status}</span><div className="actions">{r.status==="pending"&&<><button className="small-btn good" onClick={()=>review(r.id,true)}><Check/></button><button className="small-btn badbtn" onClick={()=>review(r.id,false)}><X/></button></>}</div></>}/></Card></>
}

function AdminWithdrawals({refresh}){
 const [rows,setRows]=useState([]);const load=()=>supa(supabase.from("withdrawal_requests").select("*,profiles(full_name,phone,customer_id)").order("created_at",{ascending:false})).then(setRows).catch(()=>{});useEffect(load,[refresh]);
