    ...
    try {
      ...
      if (authMode === "register") {
        ...
        if (data.user) {
          ...
          flash(p?.role === "admin" ? "Welcome to Admin." : "Welcome back.");
        }
      }
    } catch (err) {
    ...
    <div className="site">
      <header className="topbar">
        ...
        <nav>
          ...
          {isAdmin ? <button onClick={() => setPage("admin")}>Admin</button> : <button onClick={() => { setShowAdminAuth(true); setShowCustomerAuth(false); }}>Admin</button>}
      ...
      {page === "admin" && !isAdmin ? (
        <AdminGate onBack={() => setPage("home")} onSuccess={() => window.location.reload()} />
      ) : page === "admin" ? (
      ...
      {showAdminAuth && !isAdmin && (
        <AdminGate onBack={() => setShowAdminAuth(false)} onSuccess={async () => { setShowAdminAuth(false); window.location.hash = "admin"; setPage("admin"); await loadProfile((await supabase.auth.getUser()).data.user?.id); }} />
      )}
    </div>
  );
}
...
function AdminGate({ onBack, onSuccess }) {
  ...
  async function login(e) {
    ...
    try {
      ...
      const { data: p, error: roleError } = await supabase.rpc("get_my_profile");
      if (roleError) throw new Error(`Admin verification failed: ${roleError.message}`);
      if (p?.role !== "admin") {
        await supabase.auth.signOut();
  ...
  return <main className="admin-gate"><div className="login-panel"><div className="brand-mark huge">🦆</div><span className="eyebrow">PRIVATE AREA</span><h1>Admin control center</h1><p className="muted">Manage ducks, payments, customers, referrals and withdrawals.</p><form onSubmit={login}><label>Email<input type="email" autoComplete="username" value={email} onChange={e => setEmail(e.target.value)} /></label><label>Password<input type="password" autoComplete="current-password" value={password} onChange={e => setPassword(e.target.value)} /></label>{error && <div className="error">{error}</div>}<button className="btn primary wide" disabled={busy}>{busy ? "Signing in..." : "Enter Admin"}</button></form><button className="text-btn" onClick={onBack}>← Back to website</button></div></main>;
}
