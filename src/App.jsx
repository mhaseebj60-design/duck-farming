...
function AdminGate({ onBack }) {
  ...
  async function login(e) {
    ...
    if (p?.role !== "admin") { await supabase.auth.signOut(); setError("This account is not an admin."); }
    else window.location.reload();
    setBusy(false);
  }

  return <main className="admin-gate"><div className="login-panel"><div className="brand-mark huge">🦆</div><span className="eyebrow">PRIVATE AREA</span><h1>Admin control center</h1><p className="muted">Manage ducks, payments, customers, referrals and withdrawals.</p><form onSubmit={login}><label>Email<input type="email" value={email} onChange={e => setEmail(e.target.value)} /></label><label>Password<input type="password" value={password} onChange={e => setPassword(e.target.value)} /></label>{error && <div className="error">{error}</div>}<button className="btn primary wide" disabled={busy}>{busy ? "Signing in..." : "Enter Admin"}</button></form><button className="text-btn" onClick={onBack}>← Back to website</button></div></main>;
}
...
function AdminDashboard({ flash }) {
  ...
  async function approvePayment(id, approved) {
    setBusy(true);
    const { error } = approved ? await supabase.rpc("approve_payment_request", { p_request_id: id }) : await supabase.rpc("reject_payment_request", { p_request_id: id });
